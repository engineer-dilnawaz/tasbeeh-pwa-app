...
function readHasSeenGuide(): boolean {
  ...
  return window.localStorage.getItem(ADD_ITEM_GUIDE_KEY) === "1";
}
...
export interface CollectionComposerState {
  ...
  draftTarget: number;
  /** Base + per-collection custom target presets for this composer session. */
  targetPresets: number[];
  /** Whether the inline custom target editor is visible in the add-item sheet. */
  ...
  phraseQuery: string;
  /** At most one id — which phrase is highlighted in the library (radio behavior). */
...
export interface CollectionComposerActions {
  ...
  /** Radio: tap again on the sole selection clears; otherwise selects exactly one phrase. */
...
const initialComposerState = (): CollectionComposerState => ({
  ...
  hasSeenAddItemGuide: readHasSeenGuide(),
});
...
>((set, get) => ({
  ...
  markAddItemGuideSeen: () => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem(ADD_ITEM_GUIDE_KEY, "1");
    }
