...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    ...
    >
        ...
        >
          ...
          <div className="relative flex items-center justify-center w-20 h-20 rounded-full bg-base-100 border border-base-content/10 shadow-[0_8px_30px_rgb(0,0,0,0.06)] text-primary">
            {React.isValidElement(icon)
              ? // @ts-expect-error: allow styling lucide icons without over-constraining icon prop types
      ...
      <Text
        variant="heading"
        className="text-[19px] mb-2 font-bold tracking-tight text-base-content"
      >
        {title}
      </Text>
      ...
      {description && (
        <Text
          variant="body"
          color="subtle"
          className="max-w-[280px] text-[14.5px] leading-relaxed mb-6"
        >
          {description}
        </Text>
      )}
      ...
      {actionLabel && onAction && (
        ...
        >
          <Button
            variant="secondary"
            onClick={onAction}
            className="rounded-2xl px-6 h-12 shadow-sm font-semibold"
          >
            {actionLabel}
          </Button>
        </motion.div>
